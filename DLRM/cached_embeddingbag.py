...
class CacheManager(torch.nn.Module):
    ...
    @torch.no_grad()
    def none_lfu_new_prepare_ids_part_2(self, idxs_to_move_in: torch.Tensor, batch_pointer: int, data_to_receive: torch.Tensor) -> bool:
        """
        Move specific ids to specific slots according to the training plan.
        This method should be able to executed several times and it must not affect the final result as long as its return value is False.
        Args:
            idsx_to_move_in: Indices of cpu weight rows. The tensor itself is stored on GPU.
            idxs_to_update_flag: Indices of cpu weight rows that already in cache. These rows' batch flags need to be updated.
            batch_pointer: Same as the ones in prepare_ids.
            data_to_receive: Asynchronized transferred data. The transfer is started by the caller (entry loading thread).
        Returns:
            True if succeed (there are enough rows to be evicted), otherwise False.
        NOTE: This function has never been tested.
        """
        with self.timer("new_prepare_data_part_2") as gtimer:
            with self.timer("1_check_if_feasible") as timer:
                with torch.profiler.record_function("check_if_feasible"):
                    assert len(idxs_to_move_in) <= self.cuda_row_num, (
                        f"You move {len(idxs_to_move_in)} embedding rows from CPU to CUDA. "
                    ...
                    # Check if there are enough rows that can be evicted
                    num_move_in = idxs_to_move_in.numel()
                    evict_num = num_move_in - self._cuda_available_row_num
                    if evict_num > 0:
                        idx_used_gpu_rows = torch.nonzero(self.cached_idx_map != -1).squeeze(1)
                        idx_idx_evictable_gpu_rows = torch.nonzero(self.batch_flags[idx_used_gpu_rows] <= self._finished_batch).squeeze(1)

                        if idx_idx_evictable_gpu_rows.numel() < evict_num:
                            return False
                        else:
                            idx_victim_gpu_rows = idx_used_gpu_rows[idx_idx_evictable_gpu_rows[:evict_num]]
                
            if evict_num > 0:
                with self.timer("2_evict_data_begin") as timer:
                    with torch.profiler.record_function("evict_data_out_of_cache"):
                        # Evict out asynchronously
                        if self._async_copy:
                            evict_out_rows_gpu = self.cuda_cached_weight.view(self.cuda_row_num, -1).index_select(
                                0, idx_victim_gpu_rows
                            )
                        ...
                        idx_evict_cpu_row = self.cached_idx_map[idx_victim_gpu_rows]

                with self.timer("3_evict_out_finish") as timer:
                    if self.buffer_size > 0:
                        NotImplementedError()
                    else:
                    ...
                    with self.timer("3_1_evict_out_cpu_copy") as timer:
                        self.weight.view(self.num_embeddings, -1).index_copy_(0, idx_evict_cpu_row.cpu(), evict_out_rows_cpu)

                self.cached_idx_map.index_fill_(0, idx_victim_gpu_rows, -1)
                self.inverted_cached_idx.index_fill_(0, idx_evict_cpu_row, -1)
                # self.batch_flags.index_fill_(0, slots_to_evict, batch_pointer) # unnecessary
                self._cuda_available_row_num += evict_num

                weight_size = idx_victim_gpu_rows.numel() * self.embedding_dim
                self._cuda_to_cpu_numel += weight_size
                # print(f"evict embedding weight: {weight_size*self.elem_size_in_byte/1e6:.2f} MB")
                    
            with self.timer("4_evict_in") as timer:
                # Here also allocate extra memory on CUDA. #cpu_row_idxs
                if self.buffer_size > 0:
                    NotImplementedError()
                else:
                    if self._async_copy:
                        _wait_for_data(data_to_receive, self._memcpy_stream)          # if this deadlocks with evict-out transfer.....
                    else:
                        NotImplementedError()

                with self.timer("4_1_evict_in_index_copy") as timer:
                    idx_move_in_gpu_rows = torch.nonzero(self.cached_idx_map == -1).squeeze(1)[:num_move_in]
                    self.cuda_cached_weight.view(self.cuda_row_num, -1).index_copy_(0, idx_move_in_gpu_rows, data_to_receive)

            with self.timer("6_update_cache") as timer:
                self.cached_idx_map.index_copy_(0, idx_move_in_gpu_rows, idxs_to_move_in)
                self.inverted_cached_idx.index_copy_(0, idxs_to_move_in, idx_move_in_gpu_rows)
                self.batch_flags.index_fill_(0, idx_move_in_gpu_rows, batch_pointer)
                # self.freq_cnter.index_fill_(0, slots_to_move_in, 0)
                self._cuda_available_row_num -= num_move_in

            weight_size = num_move_in * self.embedding_dim
    ...
    @torch.no_grad()
    def new_prepare_ids_part_2(self, idxs_to_move_in: torch.Tensor, batch_pointer: int, data_to_receive: torch.Tensor, freq_to_move_in: torch.Tensor) -> bool:
        """
        Move specific ids to specific slots according to the training plan.
        This method should be able to executed several times and it must not affect the final result as long as its return value is False.
        Args:
        ...
        with self.timer("new_prepare_data_part_2") as gtimer:
                ...
                with torch.profiler.record_function("check_if_feasible"):
                    ...
                    if evict_num > 0:
                        ...
                        if idx_idx_evictable_gpu_rows.numel() < evict_num:
                            ...
                            idx_candidate_victim_gpu_rows = idx_used_gpu_rows[idx_idx_evictable_gpu_rows]
                            _, idx_idx_victim_gpu_rows = torch.topk(self.freq_cnter[idx_candidate_victim_gpu_rows], evict_num, largest=False)
                            idx_victim_gpu_rows = idx_candidate_victim_gpu_rows[idx_idx_victim_gpu_rows]
            ...
            with self.timer("6_update_cache") as timer:
                ...
                self.inverted_cached_idx.index_copy_(0, idxs_to_move_in, idx_move_in_gpu_rows)
                self.batch_flags.index_fill_(0, idx_move_in_gpu_rows, batch_pointer)
                self.freq_cnter.index_copy_(0, idx_move_in_gpu_rows, freq_to_move_in)
                self._cuda_available_row_num -= num_move_in
    ...
    @torch.no_grad()
    def planed_prepare_ids(self, ids_to_move_in: torch.Tensor, batch_pointer: int, slots_to_evict: torch.Tensor, slots_to_move_in: torch.Tensor, slots_to_update: torch.Tensor, evict_in_rows_gpu: torch.Tensor) -> bool:
        """
        Move specific ids to specific slots according to the training plan.
        Args:
            ids_to_move_in, batch_pointer: Same as the ones in prepare_ids.
            slots_to_evict: Indices of GPU cache rows to be evict. Given by the training plan.
            slots_to_move_in: Indices of GPU cache rows to place the ids. Given by the training plan.
            slots_to_update: Indices of GPU cache rows to update batch_flags. Given by the training plan.
        Returns:
            True if succeed, otherwise False.
        NOTE: This function has never been tested.
        """
        with self.timer("plan_guided_cache_op_2") as gtimer:
            with self.timer("1_transfer_meta_info_to_gpu") as timer:
                with torch.profiler.record_function("transfer meta info to gpu"):
                    ids_to_move_in = ids_to_move_in.to(torch.cuda.current_device())
                    slots_to_evict = slots_to_evict.to(torch.cuda.current_device())
                    slots_to_move_in = slots_to_move_in.to(torch.cuda.current_device())
                    cpu_row_idxs_to_move_in = self.idx_map.index_select(0, ids_to_move_in)

                    assert len(cpu_row_idxs_to_move_in) <= self.cuda_row_num, (
                        f"You move {len(cpu_row_idxs_to_move_in)} embedding rows from CPU to CUDA. "
                        f"It is larger than the capacity of the cache, which at most contains {self.cuda_row_num} rows, "
                        f"Please increase cuda_row_num or decrease the training batch size."
                    )
            ...
            with torch.profiler.record_function("(cache) prepare_rows_on_cuda"):
                with self.timer("prepare_rows_on_cuda") as timer:
                    ...
                    '''
                    evict_num = slots_to_evict.numel()
                    
                    
                    if evict_num > 0:
                        with self.timer("2_identify_cuda_row_idxs") as timer:
                            # Batch flag based evict strategy
                            with self.timer("2_2_check_if_ready_to_evict") as timer:
                                max_flag = torch.max(self.batch_flags[slots_to_evict])
                                if max_flag > self._finished_batch:
                                    return False
                            ...
                            evict_info = self.cached_idx_map[slots_to_evict]
                        ...
                        with self.timer("3_evict_out") as timer:
                            if self.buffer_size > 0:
                                ...
                                # TODO async gpu -> cpu
                                if self._async_copy:
                                    _wait_for_data(evict_out_rows_cpu, self._memcpy_stream)
                                else:
                                    NotImplementedError()
                            ...
                            self.cached_idx_map.index_fill_(0, slots_to_evict, -1)
                            self.inverted_cached_idx.index_fill_(0, evict_info, -1)
                            self.batch_flags.index_fill_(0, slots_to_evict, batch_pointer) # unnecessary
                            self._cuda_available_row_num += evict_num

                            weight_size = slots_to_evict.numel() * self.embedding_dim
                            self._cuda_to_cpu_numel += weight_size
                        # print(f"evict embedding weight: {weight_size*self.elem_size_in_byte/1e6:.2f} MB")
                    ...
                    with self.timer("5_evict_in") as timer:
                        ...
                        if self.buffer_size > 0:
                            ...
                            if self._async_copy:
                                _wait_for_data(evict_in_rows_gpu, self._memcpy_stream)          # if this deadlocks with evict-out transfer.....
                            else:
                                NotImplementedError()

                            with self.timer("5_3_evict_in_index_copy") as timer:
                                self.cuda_cached_weight.view(self.cuda_row_num, -1).index_copy_(0, slots_to_move_in, evict_in_rows_gpu)

                    with self.timer("6_update_cache") as timer:
                        self.cached_idx_map.index_copy_(0, slots_to_move_in, cpu_row_idxs_to_move_in)
                        self.inverted_cached_idx.index_copy_(0, cpu_row_idxs_to_move_in, slots_to_move_in)
                        self.freq_cnter.index_fill_(0, slots_to_move_in, 0)
                        self._cuda_available_row_num -= cpu_row_idxs_to_move_in.numel()

                    weight_size = cpu_row_idxs_to_move_in.numel() * self.embedding_dim
                    self._cpu_to_cuda_numel += weight_size
                    # print(f"admit embedding weight: {weight_size*self.elem_size_in_byte/1e6:.2f} MB")
            ...
            with self.timer("6_update_cache") as timer:
                with torch.profiler.record_function("6_update_cache"):
                    # Update batch flag
                    self.batch_flags.index_fill_(0, slots_to_update, batch_pointer)
        ...
        with self.timer("cache_op") as gtimer:
            ...
            with self.timer("1_identify_cpu_row_idxs") as timer:
                with torch.profiler.record_function("(cache) get unique indices"):
                    ...
                    cpu_row_idxs, repeat_times = torch.unique(self.idx_map.index_select(0, ids.view(-1)), return_counts=True)
    ...
    def _prepare_rows_on_cuda(self, cpu_row_idxs: torch.Tensor, batch_pointer: int) -> None:
        ...
        """
        evict_num = cpu_row_idxs.numel() - self._cuda_available_row_num
    ...
    @torch.no_grad()
    def none_lfu_prepare_ids(self, ids: torch.Tensor, batch_pointer: int) -> torch.Tensor:
        """
        move the cpu embedding rows w.r.t. ids into CUDA memory
        Args:
            ids (torch.Tensor): the ids to be computed
        ...
        with self.timer("cache_op") as gtimer:
            ...
            with torch.profiler.record_function("(cache) prepare_rows_on_cuda"):
                with self.timer("prepare_rows_on_cuda") as timer:
                    self.none_lfu__prepare_rows_on_cuda(comm_cpu_row_idxs, batch_pointer)
    ...
    @torch.no_grad()
    def none_lfu__prepare_rows_on_cuda(self, cpu_row_idxs: torch.Tensor, batch_pointer: int) -> None:
        """prepare rows in cpu_row_idxs on CUDA memory
        Args:
            cpu_row_idxs (torch.Tensor): the rows to be placed on CUDA
        """
        ...
        if evict_num > 0:
            with self.timer("2_identify_cuda_row_idxs") as timer:
                ...
                # Batch flag based evict strategy
                with self.timer("2_2_find_evict_gpu_idxs") as timer:
                    evict_gpu_row_idxs = self.none_lfu_find_evict_gpu_idxs(evict_num)
    ...
    @torch.no_grad()
    def none_lfu_find_evict_gpu_idxs(self, evict_num: int) -> torch.Tensor:
