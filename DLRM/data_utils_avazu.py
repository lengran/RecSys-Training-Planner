...
def processAvazuData(npzfile, convertDicts): #(d_path, d_file, npzfile, i, convertDicts, pre_comp_counts):
    ...
    if path.exists(filename):
        ...
        print("Not existing " + filename)
        with np.load(npzfile + ".npz") as data:
...
def getAvazuData(
    ...
    total_file = d_path + d_file + "_count.npz"
    if path.exists(total_file):
        with np.load(total_file) as data:
            # total_per_file = list(data["total_per_file"])
            total_count = data["total_count"]# np.sum(total_per_file)
    ...
    #                     nf.write(line)
    #             nf.close()
            total_count -= 1        # don't count the header line
        else:
    ...
    def process_one_file(
        ...
        #     convertDicts_day = [{} for _ in range(num_cat_feat)]

        with open(str(datfile)) as f:
            y = np.zeros(num_data_in_split, dtype="i4")  # 4 byte int
            X_int = np.zeros((num_data_in_split, num_con_feat), dtype="i4")  # 4 byte int
            X_cat = np.zeros((num_data_in_split, num_cat_feat), dtype="i4")  # 4 byte int
            ...
            for k, line in enumerate(f):
                ...
                hour = line[2][-2:]
                date = line[2][:-2]
                X_int[i] = np.array([date, hour], dtype=np.int32)
                ...
                #     )
                if True:
                    if line[0] in idConvertDict:
                        converted_id = idConvertDict[line[0]]
                    else:
                        idConvertDict[line[0]] = id_of_id
                        converted_id = id_of_id
                        id_of_id += 1
                ...
                if True:
                    ...
                    # debug prints
                    print(
                        "Load %d/%d  Label True: %d  Stored: %d" # "Load %d/%d  Split: %d  Label True: %d  Stored: %d"
                        % (
                            i,
                            num_data_in_split,
                            # split,
    ...
    npzfile_i = npzfile + ".npz"
    npzfile_p = npzfile + "_processed.npz"
    ...
    if recreate_flag:
        ...
        #                 convertDicts[i][j] = 1
        # else:
        if "no_multiprocessing" == "no_multiprocessing":
            total_count = process_one_file(
                npzfile + ".txt", # + "_{0}".format(i),
                npzfile,
                # i,
    ...
    if not path.exists(total_file):
        np.savez_compressed(total_file, total_count=total_count)# total_per_file=total_per_file)
        # save the id-id maper
        id_maper_file = d_path + d_file + "_id_maper.npz"
        np.savez_compressed(id_maper_file, id_id_maper=idConvertDict)
    print("Total number of samples:", total_count)
    # print("Divided into days/splits:\n", total_per_file)

    # dictionary files
    counts = np.zeros(num_cat_feat, dtype=np.int32)
    if recreate_flag:
        # create dictionaries
        for j in range(num_cat_feat):
    ...
    # else:
    #     for i in range(days):
    # processAvazuData(d_path, d_file, npzfile, i, convertDicts, counts)
    processAvazuData(npzfile, convertDicts)

    # o_file = concatAvazuData(
    #     d_path,
    #     d_file,
    #     npzfile,
    #     trafile,
