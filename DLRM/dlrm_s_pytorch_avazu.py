...
if __name__ == "__main__":
	...
	error_wrong_dataset = "This is the modified avazu version."
	...
	if (args.data_generation == "dataset"):
		sys.exit(error_wrong_dataset)
	elif (args.data_generation == "avazu"):
		train_data, train_ld, test_data, test_ld = dp.make_avazu_data_and_loaders(args)
	...
	with torch.autograd.profiler.profile(args.enable_profiling, use_gpu) as prof:
		while k < args.nepochs:
			...
			# =============================== PROFILING START ======================================
			profiling_begin = time_wrap(use_gpu)
			
			skew_table = []
			total_access = np.zeros(len(ln_emb), dtype = int)                               # counter for each embedding table number of access
			...
			if k == 0:
				...
				profiling_end = time_wrap(use_gpu)
				...
				sys.exit("Dataset pre-processing completed!!")
