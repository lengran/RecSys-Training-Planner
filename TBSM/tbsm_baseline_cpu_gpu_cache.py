...
class TBSM_Net(nn.Module):
	...
	def forward(self, x, lS_o, lS_i):
		...
		for j in range(ts - self.ts_length - 1, ts - 1):
			...
			v = self.dlrm(x[j], lS_o[j], lS_i[j], j)
		...
		w = self.dlrm(x[-1], lS_o[-1], lS_i[-1], j)
		# project onto sphere
		if self.model_type == "tsl" and self.tsl_proj:
...
# by args.save_model
def get_tbsm(args, use_gpu, gpu_cache):
...
def iterate_val_data(val_ld, tbsm, use_gpu, device):
	...
	for tmp, (X, lS_o, lS_i, T_test) in enumerate(val_ld):
		batchSize = X[0].shape[0]

		# print("----------------------[Validating (" + str(tmp) + ")]----------------------")
...
def iterate_train_data(args, train_ld, val_ld, tbsm, k, use_gpu, device, writer, losses, accuracies, isMainTraining, data_manager: DataManager):
	...
	for j, (X, lS_o, lS_i, T) in enumerate(train_ld):
		...
		if isMainTraining:
			# should_test = (
			# 	(args.test_freq > 0
			# 	and (j + 1) % args.test_freq == 0) or (j + 1 == nbatches) or (j == 0)
			# )
			should_test = j + 1 == nbatches or (j == 0)
		else:
...
def train_tbsm(args, use_gpu):
	...
	train_ld = data_manager.train_loader
	val_ld = data_manager.val_loader
	...
	# create or load TBSM
	tbsm, device = get_tbsm(args, use_gpu, data_manager.gpu_cache)
	if args.debug_mode:
...
if __name__ == "__main__":
	...
	import dlrm_baseline_alibaba_cache as dlrm
